...
public class TLTest {
    private static final Log log = LogFactory.getLog(TLTest.class);
    //泰隆银行服务请求地址
    ...
    static {
        //可以放到工程的启动类中
        SecurityUtil.initToken(TLHttpUrl, appID, appSecretKey, RSAPrivateKey, TLPublicKey);
    }
...
//        // token有效期为1小时,一小时内的业务可以用同一个token（已封装定时获取,SecurityUtil.init就会启动定时任务），如果业务接口报token失效可以调用SecurityUtil.getTLToken()更新SecurityUtil类的静态token
        if(appAccessToken==null){ System.out.println("获取token失败") ; return; }
